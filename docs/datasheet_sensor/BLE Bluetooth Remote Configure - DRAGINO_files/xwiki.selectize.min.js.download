...
require(["jquery","xwiki-selectize","xwiki-events-bridge"],function(d){if(!d.fn.xwikiSelectize.initialized){d.fn.xwikiSelectize.initialized=!0;var f=function(h,g){d(g&&g.elements||document).find(".xwiki-selectize").xwikiSelectize()};d(document).on("xwiki:dom:updated",f);d(f)}})}).apply("",[{"selectize":"/xwiki/webjars/wiki%3Axwiki/selectize.js/0.13.3/js/standalone/selectize.min"},{"selectTypedText":"Select {0} ..."}]);
